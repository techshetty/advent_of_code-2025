    ...
    public static void main(String[] args) {
        ...
        try{
            ...
            for(int x:hs1.keySet()){
                // System.out.println(x+" "+hs1.get(x).size());
                sizes.add(hs1.get(x).size());}
            Collections.sort(sizes);int len=sizes.size();
            ct=sizes.get(len-1)*sizes.get(len-2)*sizes.get(len-3);
